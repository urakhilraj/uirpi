...
if(isset($_POST["complete"])){
  ...
  if($config->get("general.initialsetup")=="0"){
    ...
    if(isset($_POST["pw"])){
      ...
      echo "Setup error - no password specified. Please create an issue <a href='' target='blank'>here</a>.";
      exit();
    }
...
<title>Acubotz Dashboard Setup</title>
...
<noscript style="z-index: 99999!important; position: absolute; top: 0; width: 98%; padding: 3%;"><div class="alert alert-danger" role="alert">Acubotz Dashboard Web Application <b>requires</b> JavaScript to be enabled in order to work properly - enable it to continue!</div></noscript>
...
<div class="container">
  ...
  <div style="margin:auto;margin-top: 30px" class="text-center">
    <img class="mb-4" src="img/acubotzlogo.png" alt="" width="72" height="72">
    <h1 class="headline h3 mb-2 font-weight-normal">Welcome!</h1>
    <h3 class="headline h5 mb-3 font-weight-light">Acubotz Dashboard</h3>
        ...
        <form class="multisteps-form__form" name="myForm">
          ...
          <div class="multisteps-form__panel shadow p-4 rounded" data-animation="scaleOut">
            ...
            <div class="multisteps-form__content">
              ...
              <p class="mb-0"><strong>Don't have access to it / forgot password?</strong><br>Alternatively, change your password manually by editing your <kbd>local.config</kbd> file.<br><a href="" target="blank" class="alert-link"><i class="bi bi-question-circle"></i>&nbsp;Help / Instructions</a></p></div>
              ...
              <div class="alert alert-warning mt-3" role="alert">
                ...
                <hr>
                <p class="mb-0"><b>In case you use remote shutdown functionality and/or have your device  accessible over Internet</b> <i>or</i> if you (more securely) want to block unwanted users you should consider <i>HTTP Authentication</i> or other mechanisms!</p>
            ...
            <div class="multisteps-form__content">
              <div class="form-row mt-4">
                ...
                <div class="list-group mb-3" style="width: 100%">
                  <a href="" class="list-group-item list-group-item-action list-group-item-<?=$pclass0;?>">
                ...
                <div class="list-group" style="width: 100%">
                  ...
                  <a href="#" data-toggle="modal" data-target="#exampleModal" data-whatever="power" class="list-group-item list-group-item-action list-group-item-<?=$pclass2;?>" role="button">
                    <div class="d-flex w-100 justify-content-between">
                      <h5 class="mb-1">Shutdown / Reboot Device remotely</h5>
                      <small class="text-info"><?=$permi2;?></small>
                    </div>
                    <p class="mb-1">Control power state of Device from your local network. Also affects inspecting (viewing) upcoming/planned power events!</p>
                    <small>Click for more</small>
                  </a>
                </div>
...
<footer class="my-5 py-3 text-muted text-center text-small">
  <p class="mb-1">@acubotz </a> by <a href="https://acutrotech.com">Acutro Technologies Pvt. Ltd.</a></p>
...
$('#exampleModal').on('show.bs.modal', function (event) {
  ...
  if(content=="power"){
    modal.find('.modal-title').text('Shutdown / Reboot device remotely (optional)')
    //text='<div class="alert alert-warning mt-3" role="alert"><i class="bi bi-info-circle"></i>&nbsp;This is <b>optional</b> and <b>recommended only</b> if your device is not accessible over the Internet!</div>In order to use the remote power functionality you have to give the user www-data advanced rights for running one specific command:<br><ul><li>Run <code>sudo visudo</code> to open the editor for adjusting user rights</li><li>Be careful what you change here! Just add the following at the end of the file: <code>www-data ALL=NOPASSWD: /sbin/shutdown</code></li><li>Restart your device and now shutdown from another device (connected to same local network like your device) is possible</li></ul><h5>Without this change</h5><ul><li>you cannot view any information about upcoming shutdown (e.g. planned via console)</li><li>you cannot shutdown your RPi remotely using the Dashboard</li></ul>';
  }else if(content=="hwinfo"){
    modal.find('.modal-title').text('Advanced hardware information (optional)');
    //text='<ul><li>Run <code>sudo usermod -aG video www-data</code> in a terminal</li></ul><div class="alert alert-info mt-3" role="alert"><i class="bi bi-info-circle"></i>&nbsp;If you do not use Raspbian (or any other RasPi distro) like Ubuntu, you do have to install libraspberrypi-bin by running <code>sudo apt install libraspberrypi-bin</code>.</div><h4>Background</h4><p>The <code>vcgencmd</code> command (specifically dedicated to device firmware) is a system command that requires certain hardware rights. Therefore one has to grant this particular right (to read hardware info) to e.g. <code>www-data</code> (under which web server is running). This is achived by adding this user to a system group called video, which the standard user device is part of by default.<br><br>In case of problems: please comment on <a href="code/Raspberry-Pi-Dashboard/issues/12" target="blank">#12</a> (or <a href="https://github.com/femto-code/Raspberry-Pi-Dashboard/issues/new" target="blank">new issue</a>)</p>';
  }
